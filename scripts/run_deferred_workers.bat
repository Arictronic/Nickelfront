...
if "%START_QWEN_WORKERS%"=="1" (
  if exist "%ROOT%scripts\run_qwen_worker.bat" (
    ...
    echo [WARN] scripts\run_qwen_worker.bat not found, Qwen gateway workers were not started.
  )
)

if "%START_CONTENT_WORKERS%"=="1" (
  if exist "%ROOT%scripts\run_worker.bat" (
    ...
    echo [WARN] scripts\run_worker.bat not found, Content workers were not started.
  )
)
...
if "%START_WORKERS%"=="1" (
  if exist "%ROOT%scripts\run_worker.bat" (
    ...
    echo [WARN] scripts\run_worker.bat not found, Celery workers were not started.
  )
)

if "%START_FLOWER%"=="1" (
  if exist "%ROOT%scripts\run_flower.bat" (
    start "Flower" cmd /k ""%ROOT%scripts\run_flower.bat""
  ) else (
    echo [WARN] scripts\run_flower.bat not found, Flower was not started.
  )
)
