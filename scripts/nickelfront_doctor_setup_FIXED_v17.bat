...
title Nickelfront Doctor + Setup FIXED v17

rem ============================================================================
rem Nickelfront Doctor + Setup
rem VERSION: 2026-05-22.17-BACKEND-RAG-PORT-WAIT
rem
...
rem   6) Не закрывает окно при ошибке, пишет логи в logs\run.
rem
rem Использование:
rem   nickelfront_doctor_setup_FIXED_v17.bat               проверка + автоустановка + запуск сервисов + финальные тесты
rem   nickelfront_doctor_setup_FIXED_v17.bat --check-only  только проверка, ничего не ставить
rem   nickelfront_doctor_setup_FIXED_v17.bat --ask         спросить перед установкой
rem   nickelfront_doctor_setup_FIXED_v17.bat --no-start-app не запускать run_all/сервисы после setup
rem   nickelfront_doctor_setup_FIXED_v17.bat --force-install принудительно переустановить зависимости
rem   nickelfront_doctor_setup_FIXED_v17.bat --recreate-venv пересоздать virtualenv
rem ============================================================================

set "SCRIPT_VERSION=2026-05-22.17-BACKEND-RAG-PORT-WAIT"
set "SCRIPT_DIR=%~dp0"
...
if exist "rag\requirements.txt" (
    call :info "rag\requirements.txt найден, но в backend-only режиме не используется. RAG работает через backend и корневой requirements.txt."
) else (
    call :info "rag\requirements.txt не найден. Это нормально для backend-only RAG."
)
...
if exist ".env" (
    for /f "usebackq eol=# tokens=1,* delims==" %%A in (".env") do (
        ...
        if /I "!K!"=="BACKEND_WAIT_SECONDS" set "BACKEND_WAIT_SECONDS=!V!"
        if /I "!K!"=="API_WAIT_SECONDS" set "BACKEND_WAIT_SECONDS=!V!"
        if /I "!K!"=="QWEN_WAIT_SECONDS" set "QWEN_WAIT_SECONDS=!V!"
        if /I "!K!"=="FRONTEND_WAIT_SECONDS" set "FRONTEND_WAIT_SECONDS=!V!"
        if /I "!K!"=="REDIS_WAIT_SECONDS" set "REDIS_WAIT_SECONDS=!V!"
...
call :strip_quotes BACKEND_WAIT_SECONDS
call :strip_quotes QWEN_WAIT_SECONDS
call :strip_quotes FRONTEND_WAIT_SECONDS
call :strip_quotes REDIS_WAIT_SECONDS
...
call :info "Ожидание портов: Redis %REDIS_WAIT_SECONDS%s, Backend %BACKEND_WAIT_SECONDS%s, Qwen %QWEN_WAIT_SECONDS%s, Frontend %FRONTEND_WAIT_SECONDS%s."
...
if errorlevel 1 (
    call :warn "pip check нашёл конфликт зависимостей. Детали в текущем логе: %LOG_FILE%."
) else (
...
if errorlevel 1 (
    call :warn "frontend\package.json читается не полностью или нет scripts. Смотри текущий лог: %LOG_FILE%."
) else (
...
if exist "rag\requirements.txt" (
    call :info "Legacy standalone rag\requirements.txt найден, но doctor/setup его не использует: RAG работает через backend."
)
exit /b 0
...
call :info "Жду старта сервисов и проверяю порты из .env. Backend ждём до %BACKEND_WAIT_SECONDS% секунд."
